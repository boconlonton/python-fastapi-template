...
def test_get_product(db: Session) -> None:
    ...
    stored_product = services.product.get(db, model_id=product.id)
...
def test_update_product(db: Session) -> None:
    ...
    product2 = services.product.update(db, db_obj=product, obj_in=product_update)
...
def test_delete_item(db: Session) -> None:
    product = utils.products.create_random_product(db)
    product2 = services.product.remove(db, model_id=product.id)
    product3 = services.product.get(db, model_id=product.id)
